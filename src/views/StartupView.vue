    ...
    <div class="h-full overflow-auto flex flex-col gap-2">
      ...
      <template
        ...
        <form class="bg-neutral p-2 rounded-btn gap-2 flex flex-col"
          @submit.prevent="() => activate(item.id, item.active)">
          <div class="flex gap-2">
            <label class="input input-bordered flex items-center gap-2 w-1/2">
              Id <span class="font-bold">{{ item.id }}</span>
            </label>
            <button type="submit" class="btn w-full flex-shrink">{{ item.active ? "DeActivate" : "Activate" }}</button>
          </div>
          <div class="flex gap-2">
            <label class="input input-bordered flex items-center gap-2 w-full">
...
export default {
  ...
  methods: {
    async create() {
      const item: UDocument<UTask> = {
        id: "", active: true, timestamp: Date.now(), utimestamp: Date.now(),
        data: {
          type: UTaskType.START,
          name: this.name,
    ...
    async activate(id: string, active: boolean) {
      updateTask(id, !active)
      const index = this.data.findIndex(item => item.id == id)
      if (index != -1) this.data[index].active = !active
    },
  ...
  beforeMount() {
    getTasks(UTaskType.START).then((querySnapshot => {
