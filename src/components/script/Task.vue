  ...
  <form class="bg-neutral p-2 rounded-btn gap-2 flex flex-col top-0" @submit.prevent="onSubmit">
    <div class="flex gap-2">
      ...
      <template v-if="task?.id">
        <button type="submit" class="btn w-1/2 btn-secondary flex-shrink">Update</button>
        <button type="button" class="btn w-1/2 btn-secondary flex-shrink"
    ...
    <div class="flex gap-2">
      <div class="flex gap-2">
        <label class="input input-bordered flex items-center gap-2 w-full">
          Server:
          <input type="text" class="grow" v-model="value.server" />
        </label>
      </div>
      <label class="label cursor-pointer w-full flex justify-between bg-base-100 p-3 rounded-btn">
        ...
        <input type="checkbox" class="toggle toggle-primary" v-model="value.isStartup" />
      </label>
      <label class="label cursor-pointer w-full flex justify-between bg-base-100 p-3 rounded-btn">
        <span class="label-text">Run With Cron</span>
        <input type="checkbox" class="toggle toggle-primary" v-model="value.isCron" />
      </label>
    </div>
    <div class="flex gap-2">
      <label class="input input-bordered flex items-center gap-2 w-full">
        Cron Expression:
        <input type="text" class="grow" v-model="value.cronExpression" :disabled="!value.isCron" />
      </label>
    </div>
    <div class="flex flex-col gap-2">
      <template v-for="command, i in value.commands">
        <div class="flex gap-2">
          <label class="input input-bordered flex items-center gap-2 w-full">
            Command:
            <input type="text" class="grow" :value="command" @input="(e) => onCommandChange(i, e)" />
          </label>
          <button type="button" class="btn w-1/4 flex-shrink" @click="() => removeCommand(i)">
      ...
      <div class="flex gap-2">
        <label class="input input-bordered flex items-center gap-2 w-full">
          New Command:
          <input type="text" class="grow" v-model="newCommand" />
        </label>
        <button type="button" class="btn w-1/4 flex-shrink" @click="addCommand">
